	...
	while (*current != '\0')
	{
		if (*current == '%')
		{
			...
			if (*current == '\0')
				...
				c = (char)va_arg(args, int), custom_putchar(c),
					count++;
			else if (*current == 's')
				...
				print_formatted_integer(format, args), count++;
			else if (*current == '%')
				custom_putchar('%'), count++;
			else if (*current == 'u' || *current == 'o' ||
				 *current == 'x' || *current == 'X')
